...
import { DriftColor, ColorPodge, CartPair } from "@hexerals/hexlib"
...
export class Coord {
    ...
    mutateScale(n: number) {
        // mutates
        this.x *= n
        this.y *= n
        return this
    }
    mutateAdd(that: Coord) {
        // mutates
        this.x += that.x
        this.y += that.y
        return this
    ...
    mutateUnit() {
        return this.mutateScale(1 / this.mag())
    }
    ...
    plus(that: Coord) {
        return this.copy().mutateAdd(that)
    }
    times(n: number) {
        return this.copy().mutateScale(n)
    }
...
export const ColorBlob = (props: ColorBlobProps) => {
    const delta = props.acceleration
        .times(-1100)
...
export class ColorBlobs extends React.PureComponent<ColorBlobsProps> {
    ...
    render(): React.ReactNode {
        ...
        return (
            ...
            >
                ...
                {this.props.colors.driftColors.map(
                    (drift: DriftColor, i: number) => (
                        <ColorBlob
                            color={drift}
    ...
    step() {
        ...
        if (elapsed >= MIN_STEP_MILLIS) {
            // limit the recalculations per second
            this.lastStep = now
    ...
    private evolve(elapsed: number) {
        if (this.props.colors.driftColors.size === 0) return
        ...
        const nReciprocal =
            1 / Math.max(1, this.props.colors.driftColors.size - 1)
        // walls
        this.log(
            `*** cycle: ${closestColors.toFixed(
                this.f
            )} to ${furthestColors.toFixed(this.f)}`
        )
        this.props.colors.driftColors.forEach((color: DriftColor) => {
            ...
            const x = false
            if (x) force.mutateAdd(this.repelSquareWalls(r, location))
            ...
            if (this.props.colors.driftColors.size >= 3)
                force.mutateAdd(
                    this.attractRepelColors(
                        color,
                        location,
                        r * COLOR_NEIGHBORHOOD,
                        nReciprocal,
                        colorDistanceReciprocal
                    )
                )
    ...
    getPosition(i: number): Coord {
        if (this.positions.has(i)) return this.positions.get(i) as Coord
        else {
    ...
    getVelocity(i: number): Coord {
        if (this.velocities.has(i)) return this.velocities.get(i) as Coord
        else {
    ...
    getAcceleration(i: number): Coord {
        if (this.accelerations.has(i)) return this.accelerations.get(i) as Coord
        else {
            const result = new Coord()
    ...
    getRadius() {
        const areaPerBlob =
            (4 * SPACE_FILL) / this.props.colors.driftColors.size
    ...
    private attractRepelColors(
        ...
        neighborhood: number,
        nReciprocal: number,
        colorDistanceReciprocal: number
    ) {
        let colorDistSum = 0
        ...
        if (colorDists.size > 1) {
            ...
            this.props.colors.driftColors.forEach((other: DriftColor) => {
                if (colorDists.has(other.key)) {
                    ...
                    const cdNorm =
                        (colorDist - colorDistMean) * colorDistanceReciprocal
                    const otherLoc = this.getPosition(other.key)
                    ...
                    this.log(
                        `     - to ${other.hslString}: norm ${cdNorm} (unit ${unit}) | raw ${colorDist} | `
                    )
                    const vec = unit.mutateScale(
                        cdNorm * nRecipActual * VELOCITY_MAX * 300
                    )
    ...
    private repelSquareWalls(radius: number, location: Coord) {
        const result = new Coord()
        // walls
        if (location.x < radius) result.x += radius - location.x
        if (location.y < radius) result.y += radius - location.y
        if (location.x + radius > 1) result.x -= location.x + radius - 1
        if (location.y + radius > 1) result.y -= radius + location.y - 1
    ...
    private repelRoundWalls(r: number, location: Coord) {
        ...
        if (d2 > 0) return location.copy().mutateScale(-Math.sqrt(d2) * 0.6)
    ...
    private static attractDarkToCenter(color: DriftColor, location: Coord) {
        ...
        return location
            .copy()
            .mutateScale(-0.3 * DriftColor.RECIP_BRIGHT * fromDark)
    }
