...
export class StatHistory {
    ...
    constructor(
        readonly values: ReadonlyArray<TurnStat>, // historical values
        readonly lastTurns: Readonly<Record<Player, number>>, // the turn when each player exited
    ...
    update = (boardState: BoardState): StatHistory => {
        ...
        if (prev.turn === boardState.turn) return this
        else {
            const next = TurnStat.create(boardState)

            // find any players that died this time
            let nextLastTurns = this.lastTurns
            if (next.hexes.size < prev.hexes.size)
                // fewer players — somebody died
                prev.hexes.stats.forEach((n: number, player: Player) => {
