...
it("makes moves", () => {
    const brt = new BoardStateReducerTester(10, 19)
...
it("specifies IQ", () => {
    expect(BasicRobot.byIntelligence(BasicRobot.MAX_IQ).skills).toEqual(
        BasicRobot.byIntelligence(BasicRobot.MAX_IQ).skills
    )
...
it("wastes not", () => {
    ...
    const downMoves = brt.moves.playerQueues.get(
        Player.Zero
    ) as List<PlayerMove>
...
const doesABeatB = (
    ...
    const brt = new BoardStateReducerTester(
        13,
        13,
        [new RandomTerrainArranger(0.3), new SpreadPlayersArranger()],
        pickNPlayers(4)
    )
    ...
    while (brt.state.turn < turnLimit) {
        ...
        if (brt.state.turn % 10 === 0 && brt.isGameOver) break
    }
...
const countAWins = (
    first: RobotMaker,
    second: RobotMaker,
    trials: number = robotTrials,
    turnLimit = 250
): number => {
    let aWins = 0
    Range(0, trials).forEach(() => {
        if (doesABeatB(first, second, turnLimit)) ++aWins
    })
    return aWins
}
...
it("control — IQ 0 vs self", () => {
    ...
    if (logWinLoss)
        ...
        console.log(
            `Dumb vs dumb: ${control}/${robotTrials} = ${control / robotTrials}`
        )
    expect(control).toBeGreaterThanOrEqual(
        robotTrials * (logWinLoss ? 0.35 : 0.3)
    )
})
...
it("IQ 1 not lose too much", () => {
    SKILL_NAMES.forEach((skillName) => {
        ...
        expect(wins).toBeGreaterThanOrEqual(
            robotTrials * (logWinLoss ? 0.4 : 0.33)
        ) // weak!
    })
})
...
it("max IQ wins a lot", () => {
    ...
    if (logWinLoss)
        // tslint:disable-next-line
        console.log(
            `Max IQ: ${wins}/${n} = ${wins / n} (${iqMax().toString()})`
        )
