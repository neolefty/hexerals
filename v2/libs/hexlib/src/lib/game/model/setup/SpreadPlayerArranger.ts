...
import { CacheDistance, HexPaths } from "../hex/ShortestPath"
...
export const PathsDM: DistanceMaker = (hexes: Set<Hex>) => new HexPaths(hexes)
...
export class SpreadPlayersArranger extends TileArranger {
    ...
    arrange(
        ...
        const emptyHexes = board.filterTiles(
        ...
        let lastNDeltas: List<number> = List<number>(
            Array(this.settleRounds).fill(Infinity)
        )
        let nRounds = 0
        ...
        while (
            ...
            (nRounds < this.maxRounds &&
                (lastNDeltas.max() as number) > this.settleDelta)
        ) {
            ...
            curStarts.forEach((tile, hex) => {
                ...
                let [nextHex, minDist] = [
                    hex,
                    distancesToOthers(hex, tile.owner).min() as number,
                ]
                hex.neighbors.forEach((neighbor) => {
                    if (!curStarts.has(neighbor) && emptyHexes.has(neighbor)) {
                        const neighborDs = distancesToOthers(
                            neighbor,
                            tile.owner
                        )
