...
import {
    FloodDM,
    PathsDM,
    SpreadPlayersArranger,
} from "../setup/SpreadPlayerArranger"
...
it("finds a slightly more complex shortest path", () => {
    ...
    const checkConnected = (a: Hex, b: Hex, path: List<Hex>) => {
        ...
        Range(0, path.size - 1).forEach((index) =>
            ...
            expect(
                (path.get(index) as Hex).neighbors.indexOf(
...
const timeArranging = (
    name: string,
    board: Board,
    arranger: TileArranger,
    log = false
) => {
    const start = Date.now()
    const status = [] as StatusMessage[]
    arranger.arrange(board, status)
    if (status.length > 0) console.log(status)
    const elapsed = Date.now() - start
    if (log)
        // tslint:disable-next-line
        console.log(
            `${name} arranger — ${board.players.size} players, ${board.edges.width} x ${board.edges.height} — ${board.hexesOccupiable.size} hexes — ${elapsed} ms`
        )
}
...
it("compares performance of flood & global", () => {
    ...
    const range = log ? Range(15, 36, 5) : Range(10, 21, 5)
    range.forEach((side) => {
        const board = Board.constructDefaultSquare(side, pickNPlayers(16), [
...
const r = (n: number, places = 2, shift = 0) =>
    Math.round(n * 10 ** shift * 10 ** places) / 10 ** places

const testPerf = (side: number, log = false) => {
    const board = Board.constructDefaultSquare(side, List(), [
        new RandomTerrainArranger(0.2),
    ])
    ...
    if (log)
        console.log(
            ...
            } | ${r(elapsed / hexes ** 2.1, 0, 6)} | ${r(
                elapsed / hexes ** 2.2,
                0,
                6
            )} | ${r(elapsed / hexes ** 2.3, 0, 6)} | ${r(
                elapsed / hexes ** 2.4,
                0,
                6
            )}`
...
it("benchmarks performance of global shortest path", () => {
    if (logBenchmarks) {
        Range(10, 36, 5).forEach((n) => {
            testPerf(n, true)
        })
        Range(37, 49, 2).forEach((n) => {
            testPerf(n, true)
        })
    } else
        Range(10, 21, 5).forEach((n) => {
            testPerf(n, false)
        })
})
