...
export class CycleReducerTester {
    ...
    get localGame(): LocalGameState | undefined {
        return this.state.localGame
    }
    ...
    get cursor(): Hex {
        return (
            (this.localBoard && this.localBoard.cursors.get(0, Hex.NONE)) ||
            Hex.NONE
        )
    }
    get edges() {
        return this.localBoard && this.localBoard.board.edges
    }
    get ll() {
        return this.edges?.lowerLeft || Hex.NONE
    }
    get ul() {
        return this.edges?.upperLeft || Hex.NONE
    }
    get ur() {
        return this.edges?.upperRight || Hex.NONE
    }
    get lr() {
        return this.edges?.lowerRight || Hex.NONE
    }
    ...
    getTile = (hex: Hex) => this.localBoard?.board.getTile(hex) || Tile.EMPTY
    ...
    queueMove = (
        ...
        this.dispatch(
            doQueueMoves(List([PlayerMove.constructDelta(player, from, delta)]))
        )
    }
    ...
    tick = (n = 1) => {
        Range(0, n).forEach(() => this.dispatch(doGameTick()))
    }
    ...
    // true by default
    useSpreadArranger = (spread = true) => {
        this.changeLocalOption("randomStart", spread ? 1 : 0)
    }
    useCornersArranger = (corners = true) => {
