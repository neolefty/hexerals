...
export const CycleReducer = (
    ...
    if (action.type === OPEN_LOCAL_GAME) return OpenLocalGameReducer(state)
        ...
        // must be a BoardStateAction, by process of elimination
        ...
        if (localGame === undefined) {
            ...
            if (newLocalBoard === localBoard) return state
            else {
...
export const ChangeLocalOptionReducer = (
    state: CycleState,
    action: ChangeLocalOptionAction
): CycleState => {
    if (state.localOptions[action.name] === action.n) return state

    const result = { ...state.localOptions }
    devAssert(action.name in result)
