...
import {
    CornersPlayerArranger,
    RandomPlayerArranger,
} from "../setup/PlayerArranger"
        ...
        .forEach((y: number) => {
            ...
            board.edges.xRange().forEach((x: number) => {
                ...
                if ((x + y) % 2 === 0 && board.inBounds(Hex.getCart(x, y))) {
                    ...
                    else c = tile.terrain === Terrain.Mountain ? "M" : "-"
                }
                line += c
            })
            out += line + "\n"
...
it("overlays", () => {
    const five = Board.constructDefaultSquare(5, pickNPlayers(4), [
        new CornersPlayerArranger(10),
    ])
    expect(five.getTile(Hex.ORIGIN)).toEqual(
        new Tile(Player.Zero, 10, Terrain.City)
    )
...
it("gets capitals", () => {
    Range(2, 10).forEach((playerCount) => {
        const ten = Board.constructDefaultSquare(
            10,
            pickNPlayers(playerCount),
...
it("navigates around a board", () => {
    ...
    expect(
        Hex.ORIGIN.plus(Hex.RIGHT_UP).plus(Hex.UP) === Hex.getCart(1, 3)
    ).toBeTruthy()
...
it("validates moves", () => {
    ...
    expect(
        threeBySeven.validate(
            PlayerMove.constructDelta(
                Player.Zero,
                threeBySeven.edges.lowerLeft,
                Hex.UP
            )
        )
    ).toBeTruthy()
    ...
    expect(
        messages[messages.length - 1].msg.startsWith("destination")
    ).toBeTruthy()
    ...
    expect(
        threeBySeven.validate(
            PlayerMove.constructDelta(Player.Zero, Hex.ORIGIN, rightUp2),
            options
        )
    ).toBeFalsy()
    ...
    // pop of only 1
    const moved = threeBySeven.applyMove(
...
it("captures capitals and checks stats", () => {
    const start = Board.constructDefaultRectangular(3, 3, pickNPlayers(2), [
    ...
    const moved = start.applyMoves(
        List([
            PlayerMove.constructDelta(
                Player.One,
                start.edges.upperRight,
                Hex.LEFT_DOWN
            ),
            PlayerMove.constructDelta(Player.One, Hex.RIGHT_UP, Hex.RIGHT_DOWN),
        ])
    ).board
    expect(moved.getCartTile(2, 0).pop).toBe(18)
    const captured = moved.applyMoves(
        List([
            PlayerMove.constructDelta(Player.Zero, Hex.ORIGIN, Hex.RIGHT_UP),
            PlayerMove.constructDelta(Player.Zero, Hex.RIGHT_UP, Hex.RIGHT_UP),
        ])
    ).board
    expect(captured.getCartTile(2, 2)).toEqual(
        new Tile(Player.Zero, 16, Terrain.CapturedCapital)
    )
    expect(captured.getCartTile(2, 0)).toEqual(new Tile(Player.Zero, 9))
