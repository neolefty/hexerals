...
): action is BoardStateAction => action.type.startsWith("boardstate ")
...
const queueMovesReducer = (
    ...
    action.moves.forEach((move: PlayerMove) => {
        ...
        const queuedTo = result.moves.playerIsQueuedTo(move.player, move.source)
        ...
        if (queuedTo)
            // if the player hopes to have already taken that hex, let them try
...
interface ApplyMovesAction {
    type: typeof APPLY_MOVES
}
...
const applyMovesReducer = (state: BoardState): BoardState => {
    ...
    const movesAndQ = state.moves.popEach((move: PlayerMove) =>
        state.board.validate(move)
    )
    if (movesAndQ) {
        // undefined if no moves to apply
...
const GameTickReducer = (state: BoardState): BoardState => {
    ...
    const result: BoardState = {
        ...
        phase:
            state.phase === GamePhase.BeforeStart
                ? GamePhase.Started
                : state.phase,
    }
    ...
    if (
        ...
        logAnalyticsEvent(
            AnalyticsAction.end,
            AnalyticsCategory.local,
            analyticsLabel(result)
        )
    }
...
const dragReducer = (state: BoardState, action: GameDragAction): BoardState => {
    const destTile = state.board.getTile(action.dest)
    if (destTile.canBeOccupied) {
        let path = floodShortestPath(
            state.board.hexesOccupiable,
            action.source,
            action.dest
        )
        ...
        if (queued && queued.size) {
            ...
            while (
                nCancel < rPath.size &&
                nCancel < rQueued.size &&
                rPath.get(nCancel) ===
                    (rQueued.get(nCancel) as PlayerMove).source
            )
                ++nCancel
        }
        if (nCancel > 0) {
...
const cancelMoveReducer = (
    state: BoardState,
    action: CancelMovesAction
): BoardState => {
    const updated: QueueAndMoves | undefined = state.moves.cancelMoves(
        action.player,
        action.cursorIndex,
        action.count
    )
...
const setCurPlayerReducer = (
    state: BoardState,
    action: SetCurPlayerAction
): BoardState => ({
    ...state,
...
export const doSetRobot = (
    player: Player,
    robot: Robot | undefined
): SetRobotAction => ({ type: SET_ROBOT, player, robot })
const setRobotReducer = (
    state: BoardState,
    action: SetRobotAction
): BoardState => ({
    ...state,
...
interface RobotsDecideAction {
    type: typeof ROBOTS_DECIDE
}
export const doRobotsDecide = (): RobotsDecideAction => ({
    type: ROBOTS_DECIDE,
})
const robotsDecideReducer = (state: BoardState): BoardState => {
    ...
    state.players.playerRobots.forEach((robot: Robot, player: Player) => {
        const decision: GameDecision | undefined = robot.decide(
            player,
            state,
            state.moves.playerQueues.get(player)
        )
        ...
        if (decision && decision.makeMoves)
            result = queueMovesReducer(
                ...
                doQueueMoves(
                    List(
                        decision.makeMoves.map((move: HexMove) =>
