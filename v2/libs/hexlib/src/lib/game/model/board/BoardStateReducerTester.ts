...
export class BoardStateReducerTester {
    ...
    constructor(
        ...
        this.dispatch(
            doNewGame(
                Board.constructDefaultRectangular(
                    width,
                    height,
                    players,
                    arrangers
    ...
    getRawTile = (coord: Hex | undefined): Tile | undefined =>
        coord && this.explicitTiles.get(coord)
    getTile = (hex: Hex): Tile => this.board.getTile(hex)
    setTile = (hex: Hex, tile: Tile) =>
        (this.state.board = this.board.setTiles(
            this.explicitTiles.set(hex, tile)
        ))
    ...
    get explicitTiles(): Map<Hex, Tile> {
        return this.board.explicitTiles
    }
    get cursors(): Map<number, Hex> {
        return this.state.cursors
    }
    get firstCursor(): Hex {
        return this.cursors.get(0, Hex.NONE)
    }
    get messages(): List<StatusMessage> {
        return this.state.messages
    }
    get cursorRawTile(): Tile | undefined {
        return this.getRawTile(this.firstCursor)
    }
    get firstCursorTile(): Tile {
        return this.getTile(this.firstCursor)
    }
    get moves(): MovementQueue {
        return this.state.moves
    }

    get ll() {
        return this.state.board.edges.lowerLeft
    }
    get ul() {
        return this.state.board.edges.upperLeft
    }
    get ur() {
        return this.state.board.edges.upperRight
    }
    get lr() {
        return this.state.board.edges.lowerRight
    }
    ...
    queueMove = (player: Player, delta: Hex, alsoCursor = true) => {
        this.dispatch(
            doQueueMoves(
                List([
                    PlayerMove.constructDelta(player, this.firstCursor, delta),
                ])
            )
        )
        if (alsoCursor) this.setCursor(this.firstCursor.plus(delta))
    }
    ...
    setCursor = (coord: Hex) => {
        this.dispatch(doPlaceCursor(coord))
    }
    doMoves = () => {
        this.dispatch(doApplyMoves())
    }
    doAllMoves = () => {
        while (this.moves.size > 0) this.doMoves()
    }
    gameTick = () => {
        this.dispatch(doGameTick())
    }
    queueRobots = () => {
        this.dispatch(doRobotsDecide())
    }
    setCurPlayer = (player: Player) => {
        this.dispatch(doSetCurPlayer(player))
    }
    ...
    cancelMoves = (
        ...
        if (actualPlayer)
            this.dispatch(doCancelMoves(actualPlayer, cursorIndex, count))
        else throw Error("current player is undefined")
    }
    ...
    popTotal = (player: Player) => {
        let result = 0
        this.board.forOccupiableTiles(
            (_, tile) => (result += tile.owner === player ? tile.pop : 0)
        )
        return result
    }
