...
export class ColorPodge {
    ...
    removeColor(idx: number): ColorPodge {
        ...
        return new ColorPodge(this.driftColors.remove(idx), this.neverSettle)
    }
    ...
    disperse(stepSize: number): ColorPodge {
        ...
        const newSettled =
            !this.neverSettle &&
            this.dispersionHistory.count((x) => x === c) >=
        ...
        let newDispHist: List<number> = this.dispersionHistory.push(
            this.closestTwo()
        )
        ...
        const newColors: List<DriftColor> = List(
            this.driftColors.map((color: DriftColor) =>
