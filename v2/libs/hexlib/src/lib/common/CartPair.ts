...
export class CartPair {
    ...
    scaleXY = (x: number, y: number) => new CartPair(this.x * x, this.y * y)
    plusXY = (x: number, y: number) => new CartPair(this.x + x, this.y + y)
    ...
    get transpose(): CartPair {
        return new CartPair(this.y, this.x)
    }
...
export class CartChain {
    static construct = (...cp: CartPair[]): CartChain => new CartChain(List(cp))
    ...
    plusXY = (x: number, y: number) => this.map((cp) => cp.plusXY(x, y))

    round = (places = 0) =>
        new CartChain(this.chain.map((pair) => pair.round(places)))
